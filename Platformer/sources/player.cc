...
sf::Vector2f Player::getCenter() const { return mPosition; }

void Player::applyVelocity(sf::Vector2f velocity) { mVelocity += velocity; }
...
void Player::draw(sf::RenderWindow &window) {
  ...
  if (false) // For debuging
  {
    ...
    shape.setPosition(mPosition.x + mCollisionRect.left,
                      mPosition.y + mCollisionRect.top);
...
bool Player::handleCollision(const sf::FloatRect &rect) {
  sf::FloatRect playerRect = {mPosition.x + mCollisionRect.left,
                              mPosition.y + mCollisionRect.top,
                              mCollisionRect.width, mCollisionRect.height};
  ...
  if (overlapx2 < minOvelap) {
    minOverlapDirection = 1;
    minOvelap = overlapx2;
  }
  if (overlapy1 < minOvelap) {
    minOverlapDirection = 2;
    minOvelap = overlapy1;
  }
  if (overlapy2 < minOvelap) {
    minOverlapDirection = 3;
    minOvelap = overlapy2;
  }
  ...
  case 0:
    ...
    if (mVelocity.y > 0 && playerRect.top < rect.top + Hooked::kMaxHookOffset &&
